...
function startRenderer(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    Portfinder.getPort(async (err, port) => {
      if (err) {
        reject(new Error(`PortError:${err}`));
      } else {
...
function electronLog(data: string | boolean, color: string) {
  if (data) {
    let log = '';
    const newData = data.toString().split(/\r?\n/);
    newData.forEach((line) => {
      log += `  ${line}\n`;
    });
    console.log(
      `${(chalk as any)[color].bold(`┏ ${config.dev.chineseLog ? '主程序日志' : 'Electron'} -------------------`)}\n\n${log}${(chalk as any)[color].bold(
        '┗ ----------------------------',
      )}\n`,
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    MainWatcher.on('event', (event) => {
      if (event.code === 'END') {
        if (electronProcess) {
          manualRestart = true;
          if (typeof electronProcess.pid === 'number' && Number.isInteger(electronProcess.pid) && electronProcess.pid > 0) {
